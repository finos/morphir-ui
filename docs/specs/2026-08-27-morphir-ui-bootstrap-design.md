...
- **Theming:** `theme-system` / `theme-light` / `theme-dark` classes on the shell root, switching CSS `color-scheme`; each token is defined once via `light-dark(light, dark)` (values verbatim, emission modernized — no generated stylesheets; system mode is native `color-scheme: light dark`).
- **Styling architecture (Svelte-idiomatic, not a port of scala's typed-style aggregation):** styles are co-located in each component's scoped `<style>` block. Global CSS is minimal — tokens, base resets, the `.no-motion` motion kill switch, and body-level drag-cursor rules. Everything scala kept in its "quarantine CSS" because kyo-ui couldn't type it (app-region drag regions, the content grid, active-nav inset shadow, gradient brand mark, resize cursors) lives in the scoped styles of the component that owns the markup. No inline `style` attributes: dynamic geometry (panel extents) flows through CSS custom properties set with Svelte `style:` directives; all rules live in stylesheets.
...
**Standing technology decision — code display and editing:** when migrated experiences need syntax highlighting or code editing, use **CodeMirror 6** for editable surfaces (TOML config editing, expression/argument editors, playgrounds; Lezer grammars for Morphir-specific syntax) and **Shiki** for read-only rendering (IR JSON, source snippets, docs — pure-JS regex engine, not the WASM Oniguruma engine, so the strict CSP holds; themed via our CSS tokens). Monaco is explicitly rejected: it was a Dioxus-era workaround in morphir-live and does not fit the bundle, CSP, or Svelte integration story. Tree-sitter is also rejected for the UI: the UI renders structured IR rather than parsing raw text, Lezer already provides editor-grade incremental parsing inside CodeMirror, and tree-sitter's WASM runtime would force `wasm-unsafe-eval` into the CSP — it belongs CLI/server-side (source-to-IR tooling), and if UI-side structural parsing is ever needed it arrives as just another service Layer. Both chosen libraries arrive as leaf components (`CodeBlock`, `CodeEditor`) — no architectural preparation is required this cycle.
